//I need a global to update a game's winner
var myGameID
        ...
        return function(e) {
            ...
            if (openCells.length > 0) {
                ...
                if (!self.gameOver) {
                    ...
                    self.titleWin();
                    gameUpdate(myGameID, p.id, self.turn+1);
                }
...
function setup() {
    if(home == true)
    {
        document.getElementById("theButton").addEventListener("click", addGame, true);
...
function addGame() {
    ...
    data = "Info=" + p1Name + ";" + p2Name;
    window.clearTimeout(timeoutID);
    makeReq("POST", "/games", 201, poller, data);
}

function gameUpdate(gameID, playerID, turns) {
    data = "Info=" + playerID + ";" + turns;
    window.clearTimeout(timeoutID);
    makeReq("PUT", "/games/" + gameID, 201, doNothing, data);
}

function poller() {
	makeReq("GET", "/games", 200, repopulate);
}

function deleteGame(gameID) {
    localStorage.removeItem("game_" + gameID)
	makeReq("DELETE", "/games/" + gameID, 204, poller);
}
...
function repopulate(responseText) {
    if(home == true)
    {
        ...
        var gamelist = JSON.parse(responseText);
        var tab = document.getElementById("theTable");
        var newRow, newCell, newButton, newGame, tempGameID;
        ...
        if(gamelist != null)
        {
            //The gamelist is one huge string atm. A split can seperate them
            for (i = 0; i < gamelist.length; i+=3) {
                newRow = tab.insertRow();
                
                newCell = newRow.insertCell();
                tempGameID = gamelist[i+1];
                newGame = document.createElement("a");
                newGame.setAttribute("href", "http://127.0.0.1:5000/game/" + tempGameID);
                newGame.innerHTML = gamelist[i];
                newCell.appendChild(newGame);
                
                if(document.getElementById(gamelist[i + 2]))    //Checks if the user created the game
                {
                    newCell = newRow.insertCell();
                    newButton = document.createElement("input");
                    newButton.type = "button";
                    newButton.value = "Delete";
                    (function(_tempGameID){ newButton.addEventListener("click", function() { deleteGame(_tempGameID); }); })(tempGameID);
        ...
        {
            newRow = tab.insertRow();
            addCell(newRow, "There are no current games.")
        }
